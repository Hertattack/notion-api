...
{
    public Func<PageObject, string, OneToManyRelationPropertyValue, bool> EdgeFilter { get; set; } =
        (_, __, ___) => true;
    ...
    public Graph Build(INotionCache notionCache, IList<NotionObject> notionObjects)
    {
        ...
        foreach (var notionPage in notionObjects.OfType<PageObject>())
        {
            ...
            foreach (var (propertyName, property) in notionPage.Properties)
            {
                ...
                if (!property.Configuration.HasValue ||
                    property.Configuration.Value is not RelationPropertyConfiguration relationPropertyConfiguration)
                    continue;
